...
const translations = {
    nl: {
        ...
        swapTitle: 'Wissel',
        floorNotice: 'Route gaat via meerdere verdiepingen — gebruik de tabs om te wisselen',
        alreadyThere: 'Je bent er al.',
        noRoute: 'Geen route gevonden.',
        chooseStartEnd: 'Kies een start- en eindpunt.',
        steps: 'stappen',
        step: 'stap',
        legendFrom: 'Van',
        legendTo: 'Naar',
        legendRoute: 'Route',
        viaStairs: (floor) => `🚶 Ga naar verdieping ${floor === 1 ? 'één' : 'begane grond'} via de trap`,
        viaLift: (floor) => `🛗 Ga naar verdieping ${floor === 1 ? 'één' : 'begane grond'} via de lift`,
        ariaNavigate: (label) => `Navigeer naar ${label}`,
        floorPlanAlt: (floor) => `Plattegrond ${floor === 0 ? 'begane grond' : 'eerste verdieping'}`,
    },
    en: {
        title: 'Floor Plan',
        from: 'From',
        ...
        swapTitle: 'Swap',
        floorNotice: 'Route spans multiple floors — use the tabs to switch',
        alreadyThere: 'You are already there.',
        noRoute: 'No route found.',
        chooseStartEnd: 'Choose a start and destination.',
        steps: 'steps',
        step: 'step',
        legendFrom: 'From',
        legendTo: 'To',
        legendRoute: 'Route',
        viaStairs: (floor) => `🚶 Go to ${floor === 1 ? 'first floor' : 'ground floor'} via the stairs`,
        viaLift: (floor) => `🛗 Go to ${floor === 1 ? 'first floor' : 'ground floor'} via the lift`,
        ariaNavigate: (label) => `Navigate to ${label}`,
        floorPlanAlt: (floor) => `Floor plan ${floor === 0 ? 'ground floor' : 'first floor'}`,
    }
}

// ─── FLOOR 0 NODES ─── (unchanged — room ids/labels stay as-is)
const NODES_0 = [
...
// Route description, now language-aware
function buildRouteMsg(path, t) {
    const msgs = []
    let prevFloor = null
    path.forEach((id) => {
        const n = nodeById(id)
        if (n.stairs && prevFloor !== null && n.floor !== prevFloor) {
            msgs.push(t.viaStairs(n.floor))
        } else if (n.lift && prevFloor !== null && n.floor !== prevFloor) {
            msgs.push(t.viaLift(n.floor))
...
function MapCombined() {
    ...
    const doNavigate = useCallback((from, to) => {
        if (from === to) { setPath([]); setRouteMsg(t.alreadyThere); return }
        const result = dijkstra(from, to)
        if (result.length > 1) {
            setPath(result)
            setRouteMsg(buildRouteMsg(result, t))
        } else {
            setPath([])
            setRouteMsg(t.noRoute)
        }
    }, [t])
                ...
                <div>
                    ...
                    <div className="map-nav-controls">
                        <div className="map-nav-group">
                            <label className="map-nav-label" htmlFor="map-from">{t.from}</label>
                        ...
                        <button className="map-swap-btn" onClick={() => { setFromId(toId); setToId(fromId) }} title={t.swapTitle}>⇄</button>

                        <div className="map-nav-group">
                            <label className="map-nav-label" htmlFor="map-to">{t.to}</label>
                    ...
                    <div className="mapSection">
                        <div className="map-img-wrap">
                            <img
                                src={floorImg}
                                alt={t.floorPlanAlt(displayFloor)}
                            ...
                            {visibleRooms.map((room) => (
                                <button
                                    ...
                                    aria-label={t.ariaNavigate(room.label)}
                                >
                        ...
                        {path.length > 1
                            ? <>
                                <span className="map-route-badge">
                                    {path.filter(id => !nodeById(id)?.corridor).length - 1} {path.length - 1 !== 1 ? t.steps : t.step}
                                </span>{' '}
                                {routeMsg}
                            </>
                            : <span className="map-route-empty">{routeMsg || t.chooseStartEnd}</span>
                        }
                    </div>

                    <div className="mapbtns">
                        <div className="map-legend">
                            <span className="map-legend-dot map-legend-dot--from" /> {t.legendFrom}
                            <span className="map-legend-dot map-legend-dot--to" /> {t.legendTo}
                            <span className="map-legend-dot map-legend-dot--path" /> {t.legendRoute}
                        </div>
                    </div>
